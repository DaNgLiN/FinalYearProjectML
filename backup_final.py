...
class Search:
    ...
    def get_text_only(self, textdata, correction=True):
        ...
        if correction:
            ...
            if anychange:
                # confirm = input("Did u mean :"+refinedquery +
                #                 " ? Enter 'y' to confirm...")
                confirm = 'n'
                if confirm == 'y':
    ...
    def query(self, q):
        ...
        if len(doclist) == 0:
            print("Not found!!")
            return
        scoring = self.partialokapi(
            doclist=doclist, nqlist=nqlist, querylen=lll)
        finallines = {}
        for doc in doclist:
        ...
        print("Found %d results in %f seconds\n" %
              (numberofdoc, s_time))
        dinfo_list.append({'numofd': numberofdoc, 'time': s_time})
        counter = 0
        for x in sortscore:
            if counter == 10:
            ...
            if x in self.doc_for_phrasequery:
                print("Query found together at linenumber(s) :",
                      self.doc_for_phrasequery[x])
            with open(x, 'r', errors='ignore') as f:
                ...
                for i in finallines[x]:
                    print_line = ''
                    line_string = xp[i-1]
                    finallist, stemmedw = self.get_text_only(
                        line_string, correction=False)
                    # print(finallist)
                    wcounter = 0
                    ...
                    ln_temp.append(i)
                    lc_temp.append(print_line)
            ln_list.append(ln_temp)
            lc_list.append(lc_temp)
            print("Score upper : ", scoring[x])
            sc_list.append(scoring[x])
            # print("\n")
            print('------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
            print('------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
            counter += 1
                    ...
                    for word in stemmedw:
                        if word in self.stemmed:
                            print_line = print_line + \
                                "\033[93m " + finallist[wcounter] + "\033[00m"
                        else:
                            print_line = print_line + " " + finallist[wcounter]
                        wcounter += 1
                    # if found:
    ...
    def partialokapi(self, doclist, nqlist, querylen, k=1.2, b=0.75, delta=1.0):
        ...
        for docid, values in doclist.items():
            ...
            for dictionarywords in values:
                ...
                denominator = len(values[dictionarywords]) + \
                    k * (1 - b + b * (doc_length / avgdl))
...
def main_search(search_str):
    ...
    dirname = './random_files_to_search/'
    ...
    while permission:
        ...
        # query = input(
        #     'Please enter your query (enclose under "." for full-phrase query)....')
        query = search_str
        if query != '':
    ...
    return [f_list, sc_list, ln_list, lc_list, dinfo_list]
...
def result(name):
    company = ['faceboook', 'google', 'youtube']
    return render_template('/results.html', com=main_search(name), searched_txt=name)
